    ...
    public class VKMessageProcessor
    {
        ...
        public static void UpdateUsers(bool firstTime = false)
        {
            ...
            newMembers.ForEach((z) =>
            {
                ...
                lock (bridgeMembers) { bridgeMembers.ForEach((x) => { lock (x.clientQueue) x.clientQueue.Add($"{IRCExtensions.GetTimeString(x)}:{z.screenName}!{(z.isBot ? "vkbot" : "vkuser")}@vkbridge-user JOIN {VKBridgeModule.cfg.ircChat} * :{z.fullName}"); }); vkMembers.Add(z); }
            ...
            hashedMembers.ToList().ForEach((z) =>
            {
                ...
                lock (bridgeMembers) { bridgeMembers.ForEach((x) => { lock (x.clientQueue) x.clientQueue.Add($"{IRCExtensions.GetTimeString(x)}:{z.Value.screenName}!{(z.Value.isBot ? "vkbot" : "vkuser")}@vkbridge-user QUIT :Goodbye!"); }); vkMembers.Remove(z.Value); }
        ...
        public static void HandleUpdate(Update u)
        {
            ...
            else if (text.StartsWith("/msg") && mS.AssertCount(3, true) && chat < 2000000000)
            {
                ...
                lock (msgReceiver.clientQueue) msgReceiver.clientQueue.Add($"{IRCExtensions.GetTimeString(msgReceiver)}:{actualSender.screenName}!{(actualSender.isBot ? "vkbot" : "vkuser")}@vkbridge-user PRIVMSG {msgReceiver.Nickname} :{message}");
            ...
            else if (act is not null && act.Type == "chat_invite_user")
            {
                ...
                lock (bridgeMembers)
                {
                    ...
                    {
                        lock (x.clientQueue) x.clientQueue.Add($"{IRCExtensions.GetTimeString(x)}:{newUser.screenName}!{(newUser.isBot ? "vkbot" : "vkuser")}@vkbridge-user JOIN {VKBridgeModule.cfg.ircChat} * :{newUser.fullName}");
                    });
                }
            }
